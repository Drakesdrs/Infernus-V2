...
void TabGUI::onRender() {
	RenderUtils::RenderText("Infernus", Vec2(12, 0), MC_Colour(255, 128, 0), 1.0f, 1.0f);
	if (Minecraft::ClientInstance() != nullptr && Minecraft::ClientInstance()->MinecraftGame()->canUseKeys()) {
		...
		if (selectedModule) {
			...
			RenderUtils::RenderText(Categories.at(currentCategory), Vec2(modulesPos.x + 2, 0), MC_Colour(255, 128, 0), 1.0f, 1.0f);
			...
			RenderUtils::FillRectangle(modulesPos, MC_Colour(90, 90, 90), .5f);
			RenderUtils::DrawRectangle(modulesPos, MC_Colour(255, 255, 255), .5f, .8f);
		}
		RenderUtils::FillRectangle(boxPos, MC_Colour(90, 90, 90), .5f);
		RenderUtils::DrawRectangle(boxPos, MC_Colour(255, 255, 255), .5f, .8f);
